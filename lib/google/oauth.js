...
async function listConnectedAccounts({ auth, req }) {
  ...
  const { data: connections, error } = await client
    .from('connections')
    .select('id, provider, account_identifier, status, scopes, metadata, created_at, updated_at')
  ...
  const accounts = (connections || []).map(conn => {
    ...
    const source = (driveSources || []).find(ds => ds.account_id === conn.id) || (driveSources && driveSources[0]);
    return {
      id: conn.id,
      provider: conn.provider,
      nickname: source?.nickname || (conn.provider === 'google' ? "Nad's Google Workspace" : conn.provider),
...
module.exports = { buildOAuthStartResponse, handleOAuthCallback, listConnectedAccounts };
